...
namespace MFST
{
	struct MfstState							   //состояние автомата(для сохранения
	{
		...
		MFSTSTSTACK st;							   //стек автомата с содержимым на момент разбора
		MfstState();
		MfstState(
	...
	struct Mfst									   //магазинный автомат
	{
		enum RC_STEP								//код возврата функции step,
		{
			...
			SURPRICE								//неожиданный код возврата ( ошибка в step)
		};
		...
		Scanner::Tables Scanner;							 //результат работы лексического анализатора
		MFSTSTSTACK st;								 //стек автомата	
		std::stack<MfstState> storestate;			 //стек для сохранения состояний
		Mfst();
		Mfst(
			Scanner::Tables plex,						   //результат работы лексического анализатора
			GRB::Greibach pgrebach);				   //грамматика Грейбах
		char* getCSt(char* buf);					   //получить содержимое стека
		char* getCLenta(char* buf, short pos, short n = 25);//лента: n символов с pos
		char* getDiagnosis(char* buf);				//получить n-ую строку диагностики или 0х00
